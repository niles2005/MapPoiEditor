$(document).ready(function() {
  ...
  var createMarkerIcon = new qq.maps.MarkerImage(
    "images/create.png",
    null,
    null,
    null,
  ...
  $("body").on("shown.bs.modal", ".modal", function(e) {
    $(this)
      ...
      .focus();
    $(this)
      .find(".open-scroll .mask.selected")
      .scrollintoview();
  });
  ...
  function createPoi(latLng) {
    $.getJSON(
      "service?name=createpoi&groupkey=" +
        currentGroup.key +
        "&v=" +
        new Date().getTime(),
      function(ret) {
  ...
  $("#pageConfig").click(function() {
    ...
    for (let i = 0; i < datas.groups.length; i++) {
      ...
      let row =
        ...
        groupCheck +
        "</td>" +
        "<td><img class='mapIcon' src='" +
        poiGroup.markerPath +
        poiGroup.markerImage +
        "'>" +
        ...
        poiGroup.markerImage +
        "'></td>" +
        "<td><img class='listImage' src='" +
        poiGroup.picturePath +
        poiGroup.pictureImage +
        "'></td>" +
  ...
  $("#groupSave").click(function() {
    ...
    if (configGroup) {
      ...
      $tr
        .find("img.mapIcon")
        .eq(0)
        .attr("src", configGroup.markerPath + configGroup.markerImage);
      $tr
        .find("img.mapIcon")
        .eq(1)
        .attr(
          "src",
          configGroup.markerPath + "focus" + configGroup.markerImage
        );
      $tr
        .find("img.listImage")
        .attr("src", configGroup.picturePath + configGroup.pictureImage);
    } else {
      //new
      $.getJSON(
        "service?name=createpoigroup&groupname=" +
          groupName +
          "&v=" +
          new Date().getTime(),
        function(ret) {
  ...
  function loadCoverImages(imageName) {
    ...
    $coverImages.empty();
    $.getJSON(
      "service?name=images&type=cover&v=" + new Date().getTime(),
      function(ret) {
        if (ret.retCode >= 0 && ret.data) {
  ...
  function loadPOIGroupConfigImages() {
    ...
    $pictureImages.empty();
    $.getJSON(
      "service?name=images&type=group&v=" + new Date().getTime(),
      function(ret) {
        if (ret.retCode >= 0 && ret.data) {
